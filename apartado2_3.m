...
%f2 = figure(2) 
for alpha=linspace(2,13,n)% cambiar alpha por  parametro a variar y cambiar (2,13) por rango adecuado
...
for pi_f=linspace(0.8,1.59,n) % cambiar parametro a variar
...
for pi_f=linspace(0.8,1.59,n) % cambiar parametro a variar
   [tsp,isp] = turbofan_analysis(alpha_0, pi_f_0, pi_LPC_0, pi_HPC_0, convergent_only); % quitar "_0" del parametro a variar
   scatter(ax1,pi_f, tsp, 'filled', 'b')% cambiar alpha por parametro
   hold on
   scatter(ax2,pi_f, isp, 'filled', 'g')% cambiar alpha por parametro
   hold on
